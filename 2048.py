...
tuplo_d_2= (('N', range(1, 5)), ('S', range(4, 0, -1)), ('W', range(1, 5)), ('E', range(4, 0, -1)))
...
def tabuleiro_reduz(t, d):
    ...
    for i in range(len(tuplo_d)):
        if d == tuplo_d_2[i][0]:
            lista_aux = []
            for parcela1 in tuplo_d_2[i][1]:
                for parcela2 in tuplo_d_2[i][1]:
                    tuplo_d_3 = (('N', range(1, 5), cria_coordenada(parcela2, parcela1), lista_aux[parcela2 - 1]), ('S', range(4, 0, -1), cria_coordenada(parcela2, parcela1), lista_aux[len(lista_aux) - parcela2]), ('W', range(1, 5), cria_coordenada(parcela1, parcela2), lista_aux[parcela2 - 1]), ('E', range(4, 0, -1), cria_coordenada(parcela1, parcela2),  lista_aux[len(lista_aux) - parcela2]))
                    lista_aux = lista_aux + [tabuleiro_posicao(t, tuplo_d_3[i][2])]
                lista_aux = filtra(lista_aux)
                lista_aux = junta(lista_aux, t)
                lista_aux = filtra(lista_aux)
                for parcela2 in tuplo_d_2[i][1]:
                    tuplo_d_4 = (('N', range(1, 5), cria_coordenada(parcela2, parcela1), lista_aux[parcela2 - 1]), ('S', range(4, 0, -1), cria_coordenada(parcela2, parcela1), lista_aux[len(lista_aux) - parcela2]), ('W', range(1, 5), cria_coordenada(parcela1, parcela2), lista_aux[parcela2 - 1]), ('E', range(4, 0, -1), cria_coordenada(parcela1, parcela2),  lista_aux[len(lista_aux) - parcela2]))
                    tabuleiro_preenche_posicao(t, tuplo_d_4[i][2], tuplo_d_4[i][3])
                lista_aux = []
            return t
...
def e_tabuleiro(possivel_t):
    ...
    if isinstance(possivel_t, dict) and len(possivel_t)==17 and ('pontuacao' in possivel_t):
        for l in range(1,5):
            for c in range(1,5):
                if cria_coordenada(l,c) not in possivel_t:
                    return False
        return True
    else:
        return False
...
def jogo_2048():
    '''Esta funcao corresponde a funcao principal do jogo. Nao recebe qualquer argumento, e permite a um utilizador jogar um jogo completo de 2048. Em cada turno '''
